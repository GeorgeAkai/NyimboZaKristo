...
interface EnglishHymnListProps {
  hymns: EnglishHymn[]
  search: string
  activeCategory: EnglishHymn['category'] | 'All'
  onSearchChange: (value: string) => void
  onCategoryChange: (category: EnglishHymn['category'] | 'All') => void
  onSelectHymn: (hymn: EnglishHymn) => void
}

const suggestedCategories = ['All', 'Sabbath', 'Praise', 'Worship'] as const

export function EnglishHymnList({
  hymns,
  search,
  activeCategory,
  onSearchChange,
  onCategoryChange,
  onSelectHymn,
}: EnglishHymnListProps) {
  return (
    <section className="space-y-6">
      <div className="rounded-2xl bg-navy-900 p-5 text-white shadow-reverent dark:bg-navy-950">
        <h1 className="font-display text-2xl font-bold">English Hymns</h1>
        <p className="mt-2 text-sm text-slate-200">
          Lyrics from gccsatx.com. Each song offers up to five videos of the same hymn from Daniel
          Baptist, Classic Hymns, Kaleb Brasee, Tabernacle Choir, and other choirs.
        </p>
